...
#### Exposing via Ingress (optional)

By default, the metering service is only reachable from within the cluster. If an external system (such as a billing
platform, BI tool, or custom integration) needs to query the metering API directly, expose the service through an ingress.
...
Requests made through the ingress still require the Usage Aggregator service credentials. See [Accessing Aggregated Data](#accessing-aggregated-data).
...
The metering service is reachable in-cluster through the `usage-aggregator` Kubernetes service (default port 5800), or externally
through the configured ingress hostname when [exposed via ingress](#exposing-via-ingress-optional).
